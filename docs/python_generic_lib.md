...
An instance of the ``` SimpleCalculatorGroupController ``` class can be accessed from the API Client.

```python
 simple_calculator_group_client = client.simple_calculator_group
```
...
result = simple_calculator_group_client.calculate(operation, x, y)
